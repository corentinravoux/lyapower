...
class MyClass(object):
    ...
    def write_pk_tk(self,z,name,kmin=-4,kmax= 3,nb_points=2000,verbose=True):
        ...
        """
        self.model.compute()
        Power,sigma_8 = self.write_pk(name,z,kmin=kmin,kmax=kmax,nb_points=nb_points,verbose=verbose)
    ...
    def write_pk(self,name,z,kmin=-4,kmax=3,nb_points=2000,header_output=True,verbose=True):
        if(self.output_format == "class"):
            (k_space,Pk,sigma_8) = self.compute_power_spectrum(z,kmin=kmin,kmax=kmax,nb_points=nb_points,verbose=verbose) # k in h/Mpc
            ...
            (k_space,Pk,sigma_8) = self.compute_power_spectrum(z,k_array=kpower,verbose=verbose)
